...
async function seedDemo() {
    ...
    const cash = await ensureRecord(prisma.paymentOption, { title: 'Cash' }, {
        title: 'Cash',
        abbreviation: 'CASH',
    ...
    const bankTransfer = await ensureRecord(prisma.paymentOption, { title: 'Bank Transfer' }, {
        title: 'Bank Transfer',
        abbreviation: 'BANK',
    ...
    const cbeAccount = await ensureRecord(prisma.bankAccount, { accountNumber: '100000000001' }, {
        bankId: cbeBank.id,
    ...
    const dell = await ensureRecord(prisma.brand, { title: 'Dell' }, {
        title: 'Dell',
        abbreviation: 'DELL',
    ...
    const samsung = await ensureRecord(prisma.brand, { title: 'Samsung' }, {
        title: 'Samsung',
        abbreviation: 'SMSG',
    ...
    const apple = await ensureRecord(prisma.brand, { title: 'Apple' }, {
        title: 'Apple',
        abbreviation: 'AAPL',
    ...
    const mainWarehouse = await ensureRecord(prisma.location, { title: 'Main Warehouse' }, {
    ...
    const electronics = await ensureRecord(prisma.typeOfProduct, { title: 'Electronics' }, {
        title: 'Electronics',
        abbreviation: 'ELEC',
    ...
    const wholesaler = await ensureRecord(prisma.supplier, { title: 'Addis Tech Distributors' }, {
    ...
    const purchaseSource = await ensureRecord(prisma.stockSource, { title: 'Supplier Purchase' }, {
    ...
    const scheduledStatus = await ensureRecord(prisma.shipmentStatusOption, { title: 'Scheduled' }, {
        title: 'Scheduled',
        abbreviation: 'SCH',
    ...
    const inTransitStatus = await ensureRecord(prisma.shipmentStatusOption, { title: 'In Transit' }, {
        title: 'In Transit',
        abbreviation: 'TRN',
    ...
    const deliveredStatus = await ensureRecord(prisma.shipmentStatusOption, { title: 'Delivered' }, {
        title: 'Delivered',
        abbreviation: 'DLV',
    ...
    const packedItemStatus = await ensureRecord(prisma.shipmentItemStatusOption, { title: 'Packed' }, {
        title: 'Packed',
        abbreviation: 'PCK',
    ...
    const deliveredItemStatus = await ensureRecord(prisma.shipmentItemStatusOption, { title: 'Delivered' }, {
        title: 'Delivered',
        abbreviation: 'DLV',
    ...
    const carrier = await ensureRecord(prisma.carrier, { title: 'Ethio Rapid Logistics' }, {
    ...
    const customerOne = await ensureRecord(prisma.customer, { phoneNumber: '0911223344' }, {
    ...
    const customerTwo = await ensureRecord(prisma.customer, { phoneNumber: '0911556677' }, {
    ...
    const laptopProduct = await ensureRecord(prisma.product, { title: 'Laptop' }, {
    ...
    const phoneProduct = await ensureRecord(prisma.product, { title: 'Smartphone' }, {
        typeOfProductId: electronics.id,
        title: 'Smartphone',
        abbreviation: 'PHN',
    ...
    const tabletProduct = await ensureRecord(prisma.product, { title: 'Tablet' }, {
        typeOfProductId: electronics.id,
        title: 'Tablet',
        abbreviation: 'TAB',
    ...
    const monitorProduct = await ensureRecord(prisma.product, { title: 'Monitor' }, {
        typeOfProductId: electronics.id,
        title: 'Monitor',
        abbreviation: 'MTR',
    ...
    const smartwatchProduct = await ensureRecord(prisma.product, { title: 'Smartwatch' }, {
        typeOfProductId: electronics.id,
        title: 'Smartwatch',
        abbreviation: 'SWT',
    ...
    const laptopVariant = await ensureRecord(prisma.productVariant, { code: 'DELL-LAT-5440-16-512' }, {
    ...
    const phoneVariant = await ensureRecord(prisma.productVariant, { code: 'SMSG-A54-8-128-BLK' }, {
    ...
    const tabletVariant = await ensureRecord(prisma.productVariant, { code: 'APPL-IPAD10-64-SLV' }, {
    ...
    const monitorVariant = await ensureRecord(prisma.productVariant, { code: 'DELL-P2723DE-27-QHD' }, {
    ...
    const smartwatchVariant = await ensureRecord(prisma.productVariant, { code: 'SMSG-GW6-44-BLK' }, {
    ...
    await ensureRecord(prisma.productVariantAttribute, { productVariantId: laptopVariant.id, key: 'RAM' }, {
    ...
    await ensureRecord(prisma.productVariantAttribute, { productVariantId: laptopVariant.id, key: 'Storage' }, {
        productVariantId: laptopVariant.id,
        key: 'Storage',
        value: '512GB SSD',
        isEnabled: true,
    ...
    await ensureRecord(prisma.productVariantAttribute, { productVariantId: phoneVariant.id, key: 'Color' }, {
    ...
    await ensureRecord(prisma.productVariantAttribute, { productVariantId: tabletVariant.id, key: 'Storage' }, {
    ...
    await ensureRecord(prisma.productVariantAttribute, { productVariantId: monitorVariant.id, key: 'Size' }, {
    ...
    await ensureRecord(prisma.productVariantAttribute, { productVariantId: smartwatchVariant.id, key: 'Color' }, {
    ...
    const laptopPrice = await ensureRecord(prisma.productVariantPrice, { productVariantId: laptopVariant.id, tag: 'retail' }, {
        productVariantId: laptopVariant.id,
        listPrice: 85000,
        currency: 'ETB',
        tag: 'retail',
    ...
    const phonePrice = await ensureRecord(prisma.productVariantPrice, { productVariantId: phoneVariant.id, tag: 'retail' }, {
        productVariantId: phoneVariant.id,
        listPrice: 25000,
        currency: 'ETB',
        tag: 'retail',
    ...
    const tabletPrice = await ensureRecord(prisma.productVariantPrice, { productVariantId: tabletVariant.id, tag: 'retail' }, {
        productVariantId: tabletVariant.id,
        listPrice: 42000,
        currency: 'ETB',
        tag: 'retail',
    ...
    await ensureRecord(prisma.productVariantPrice, { productVariantId: monitorVariant.id, tag: 'retail' }, {
        productVariantId: monitorVariant.id,
        listPrice: 28000,
        currency: 'ETB',
        tag: 'retail',
    ...
    await ensureRecord(prisma.productVariantPrice, { productVariantId: smartwatchVariant.id, tag: 'retail' }, {
        productVariantId: smartwatchVariant.id,
        listPrice: 18000,
        currency: 'ETB',
        tag: 'retail',
    ...
    const laptopStock = await ensureRecord(prisma.stock, { batchId: 'BATCH-LAPTOP-001' }, {
        productVariantId: laptopVariant.id,
    ...
    const phoneStock = await ensureRecord(prisma.stock, { batchId: 'BATCH-PHONE-001' }, {
        productVariantId: phoneVariant.id,
        supplierId: wholesaler.id,
        locationId: mainWarehouse.id,
    ...
    await ensureRecord(prisma.stock, { batchId: 'BATCH-TABLET-001' }, {
        productVariantId: tabletVariant.id,
        supplierId: wholesaler.id,
        locationId: mainWarehouse.id,
    ...
    await ensureRecord(prisma.stock, { batchId: 'BATCH-MONITOR-001' }, {
        productVariantId: monitorVariant.id,
        supplierId: wholesaler.id,
        locationId: mainWarehouse.id,
    ...
    await ensureRecord(prisma.stock, { batchId: 'BATCH-WATCH-001' }, {
        productVariantId: smartwatchVariant.id,
        supplierId: wholesaler.id,
        locationId: mainWarehouse.id,
    ...
    const completedOrder = await ensureRecord(prisma.order, { orderNumber: 1001 }, {
    ...
    const pendingOrder = await ensureRecord(prisma.order, { orderNumber: 1002 }, {
    ...
    const completedLaptopItem = await ensureRecord(prisma.orderItem, { orderId: completedOrder.id, productVariantId: laptopVariant.id }, {
    ...
    const completedPhoneItem = await ensureRecord(prisma.orderItem, { orderId: completedOrder.id, productVariantId: phoneVariant.id }, {
        orderId: completedOrder.id,
        productVariantId: phoneVariant.id,
    ...
    await ensureRecord(prisma.orderItem, { orderId: pendingOrder.id, productVariantId: tabletVariant.id }, {
    ...
    const completedPayment = await ensureRecord(prisma.payment, { referenceNumber: 'PAY-1001' }, {
        amount: 135000,
        unassignedAmount: 0,
        isEnabled: true,
        enabledById,
    ...
    await ensureRecord(prisma.orderPayment, { orderId: completedOrder.id, paymentId: completedPayment.id }, {
        orderId: completedOrder.id,
    ...
    const laptopFulfillment = await ensureRecord(prisma.orderItemFulfillment, { orderItemId: completedLaptopItem.id, stockId: laptopStock.id }, {
    ...
    const phoneFulfillment = await ensureRecord(prisma.orderItemFulfillment, { orderItemId: completedPhoneItem.id, stockId: phoneStock.id }, {
    ...
    const shipment = await ensureRecord(prisma.shipment, { shipmentNumber: 'SHP-1001' }, {
    ...
    await ensureRecord(prisma.shipmentStatus, { shipmentId: shipment.id, statusId: scheduledStatus.id }, {
        shipmentId: shipment.id,
        statusId: scheduledStatus.id,
        isEnabled: true,
        enabledById,
    }, 'shipment status scheduled for SHP-1001');
    await ensureRecord(prisma.shipmentStatus, { shipmentId: shipment.id, statusId: inTransitStatus.id }, {
        shipmentId: shipment.id,
        statusId: inTransitStatus.id,
        isEnabled: true,
        enabledById,
    }, 'shipment status in transit for SHP-1001');
    await ensureRecord(prisma.shipmentStatus, { shipmentId: shipment.id, statusId: deliveredStatus.id }, {
        shipmentId: shipment.id,
        statusId: deliveredStatus.id,
        isEnabled: true,
        enabledById,
    }, 'shipment status delivered for SHP-1001');
    const laptopShipmentItem = await ensureRecord(prisma.shipmentItem, { shipmentId: shipment.id, orderItemFulfillmentId: laptopFulfillment.id }, {
        shipmentId: shipment.id,
    ...
    const phoneShipmentItem = await ensureRecord(prisma.shipmentItem, { shipmentId: shipment.id, orderItemFulfillmentId: phoneFulfillment.id }, {
        shipmentId: shipment.id,
    ...
    await ensureRecord(prisma.shipmentItemStatus, { shipmentItemId: laptopShipmentItem.id, statusId: packedItemStatus.id }, {
        shipmentItemId: laptopShipmentItem.id,
        statusId: packedItemStatus.id,
        isEnabled: true,
        enabledById,
    }, 'shipment item packed status for laptop');
    await ensureRecord(prisma.shipmentItemStatus, { shipmentItemId: laptopShipmentItem.id, statusId: deliveredItemStatus.id }, {
        shipmentItemId: laptopShipmentItem.id,
        statusId: deliveredItemStatus.id,
        isEnabled: true,
        enabledById,
    }, 'shipment item delivered status for laptop');
    await ensureRecord(prisma.shipmentItemStatus, { shipmentItemId: phoneShipmentItem.id, statusId: packedItemStatus.id }, {
        shipmentItemId: phoneShipmentItem.id,
        statusId: packedItemStatus.id,
        isEnabled: true,
        enabledById,
    }, 'shipment item packed status for phone');
    await ensureRecord(prisma.shipmentItemStatus, { shipmentItemId: phoneShipmentItem.id, statusId: deliveredItemStatus.id }, {
        shipmentItemId: phoneShipmentItem.id,
        statusId: deliveredItemStatus.id,
        isEnabled: true,
        enabledById,
